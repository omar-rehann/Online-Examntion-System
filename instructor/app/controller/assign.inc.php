...
if (isset($_GET['assignToGroup'])) {
    ...
    if ($duration == 0 || !is_numeric($duration)) {
        echo 'Test Duration must be a valid Number greater than Zero';
    } elseif ($percent == 0 || !is_numeric($percent)) {
        echo 'Pass Percent must be a valid Number greater than Zero';
        ...
        $newAssign->createSetting($testID, $startTime, $endTime, $prevQuestion, $duration, $percent, $viewAnswers, $releaseResult, $sendToS, $sendToI);
        $newAssign->AssignToGroup($groupID, $testID);
        header("Location: ../../?groups");
        exit;
    }
} elseif (isset($_GET['deleteAssignedTest'])) {
