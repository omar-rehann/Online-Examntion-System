...
if ($_SESSION['mydata']->isAdmin) {
    if (isset($_GET['suspendStudent'])) {
        ...
        $password = isset($_POST['password']) ? $_POST['password'] : null; // التعديل هنا
      ...
      $data = array("student ID, Name, Email Address, Phone Number, Password");
      foreach ($students as $std) {
          $line = $std->id . ',' . $std->name . ',' . $std->email . ',' . $std->phone . ',' . ($std->password ?? '');
      ...
      fclose($fp);
        ...
        foreach ($sheetData as $row) {
            ...
            $password = !empty($row['E']) ? $row['E'] : null; // التعديل هنا
